...
- **服务与快捷方式上桌面（融合 watchcow + watchcow-proxy）**：
  - **Docker 容器与本机端口一键放桌面**：在端口列表中点击高亮“放到桌面”，即刻在飞牛OS桌面生成官方原生应用图标；自动基于 Homarr CDN 匹配官方精美服务图标，智能降级至通用容器图标，杜绝误用主产品图标。
  - **包名绝对隔离与多快捷方式共存**：每项桌面图标均生成全局唯一隔离的应用包标识（`fndocker.<name>-<id>`），彻底解决同一端口或名称导致的覆盖与卸载冲突。
  ...
  - **桌面图标统一管理与防并发即时开关**：内置防重并发保护与 iOS 风格滑动开关，切换时提供清晰的“处理中”状态提示，随时启用/停用桌面图标（停用时桌面图标隐去，启用时立刻恢复），支持编辑与移出桌面。
- **8 天本地滚动日志与启动诊断**：
  - **系统级日志记录**：自动按天轮转记录于文件系统 `${TRIM_PKGVAR}/logs/app-YYYY-MM-DD.log` 及 `${TRIM_PKGVAR}/info.log`。
  - **8 天智能自清理**：后台自动修剪清理超过 8 天的历史日志文件，保持磁盘整洁。
  - **产品界面日志视窗**：Web 界面内置终端风格日志查看器，支持按日期切换、级别过滤（全部/信息/警告/错误）、关键字高亮检索与一键下载。
  - **启动自动对齐与多路径双写兼容**：启动时自动校验并修复所有已保存项的唯一包标识，自动通过 `appcenter-cli` 补全注册；打包严格兼容 `desktop_uidir=ui` 与 `app/ui` 双路径，图标支持横杠与下划线多尺寸规范。
